...
impl VideoStream {
    ...
    pub fn load(path: &std::path::Path) -> Option<Self> {
        ...
        // + caps filter forcing BGRA + appsink the compositor pulls
        // decoded frames from. BGRA maps to `Fourcc::Argb8888` on
        // little-endian so the buffer can go straight to
        // `TextureBuffer::from_memory`.
        //
        // `max-buffers=2 drop=true` on the appsink keeps the
        // per-sink queue tight while letting preroll land at least
        // one buffer. Earlier attempts with an added `queue` +
        // `max-buffers=1` stalled preroll on some files so playback
        // froze on the first frame; keeping this path minimal is
        // safer — playbin3's internal queues already handle most
        // of the backpressure between decode and sink.
